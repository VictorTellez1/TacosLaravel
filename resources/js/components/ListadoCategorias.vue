<template>
    <div>
        <nav class="d-flex flex-column flex-lg-row container justify-content-lg-center">
            <a
...
export default {
    created(){
        axios.get('/api/categorias')
            .then(respuesta=>{
                this.$store.commit('AGREGAR_CATEGORIAS',respuesta.data) //Esto es lo que se madna al store por eso se usa respuesta.data
            })
    },
    computed:{
        categorias(){
            return this.$store.getters.obtenerCategorias //Se sacan del store
        }
    },
    methods:{
